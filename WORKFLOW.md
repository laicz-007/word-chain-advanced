...
> **2026-09 词库大修**：全量库从 281,878 涨到 **307,113**（+25,235）。原因是**旧 `db.json` 里积累了大量误删** ——
> 它被更早版本的过滤规则改写过，而 `compute_chain_idx.js` 是"就地改写"，删掉的词再也回不来。
> 那次大修的完整经过见第 3 节 #14/#15 与第 4 节 Phase 3。
...
> 本项目已经因为同一类问题踩过三次坑（`[域]` 误伤 to/be/as、`昵称` 误伤 bunny、本次的 部/国）。
> **判据应当是"这个词条本身是不是"，而不是"释义里有没有提到"。**
